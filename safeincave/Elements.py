...
# 		self.qsi_v = to.zeros(self.n_elems, dtype=to.float64)
# 		self.qsi_v_old = to.zeros(self.n_elems, dtype=to.float64)
...
# 		self.Q = (eps_ie_rate_eps - self.eps_ie_rate) / EPSILON_ALPHA
...
# 		self.qsi_v = self.qsi_v_old + (eps_rate[:,0,0] + eps_rate[:,1,1] + eps_rate[:,2,2])*dt
# 		# print(float(self.qsi[0]), (eps_rate[:,0,0] + eps_rate[:,1,1] + eps_rate[:,2,2]), float(self.qsi_v[0]))
# 		return alpha - self.a_1 / (((self.a_1/self.alpha_0)**(1/self.eta) + self.qsi)**self.eta)

# 	def compute_eps_bar(self, phi1, phi2):
# 		self.eps_bar = self.eps_ie_old + phi1*self.eps_ie_rate_old + phi2*self.eps_ie_rate

# 	def compute_eps_ie(self, stress, stress_k, phi2):
# 		self.eps_ie = self.eps_bar + phi2*dotdot2(self.G, stress - stress_k) - self.B
...
# 		self.qsi_v_old = self.qsi_v.clone()
...
# 	def compute_alpha_q(self, alpha):
# 		# alpha_q = alpha + self.k_v*(self.alpha_0 - alpha)*(1 - self.qsi_v/self.qsi)
# 		self.qsi[self.qsi < 1e-12] = 1e-12
# 		alpha_q = alpha + self.k_v*(self.alpha_0 - alpha)*(1 - self.qsi_v/self.qsi)
# 		return alpha_q

# 	def compute_dQdS(self, alpha, stress_components, stress_invariants):
# 		s_xx, s_yy, s_zz, s_xy, s_xz, s_yz = stress_components
# 		I1, I2, I3, J2, J3, Sr, I1_star = stress_invariants
...
# 		dQdS = to.zeros((self.n_elems, 3, 3), dtype=to.float64)
# 		dQdS[:,0,0] = dQdS_00
# 		dQdS[:,1,1] = dQdS_11
# 		dQdS[:,2,2] = dQdS_22
...
# 		alpha_q = self.compute_alpha_q(alpha)
...
# 		# print(float(alpha[0]), float(alpha_q[0]), float(self.qsi[0]), float(self.qsi_v[0]))
# 		stress_components = s_xx, s_yy, s_zz, s_xy, s_xz, s_yz
# 		stress_invariants = I1, I2, I3, J2, J3, Sr, I1_star
# 		# dQdS = self.compute_dQdS(alpha, stress_components, stress_invariants)
# 		dQdS = self.compute_dQdS(alpha_q, stress_components, stress_invariants)
