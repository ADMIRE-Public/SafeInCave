...
class ScreenPrinter:
    ...
    def print_mesh_info(self) -> None:
        ...
        self.set_header_columns(
            ["# of elements", "# of nodes", "Location" + size * " "], "left"
        )
        self.print_header()
        self.set_row_formats(["%.i", "%.i", "%s"], ["left", "left", "left"])
        n_elems = MPI.COMM_WORLD.allreduce(
            self.grid.mesh.topology.index_map(self.grid.domain_dim).size_local,
            op=MPI.SUM,
        )
        n_nodes = MPI.COMM_WORLD.allreduce(
            self.grid.mesh.topology.index_map(0).size_local, op=MPI.SUM
        )
    ...
    def print_partition_info(self) -> None:
        ...
        for rank in range(1, comm.Get_size()):
            if comm.rank == rank:
                n_elems = self.grid.mesh.topology.index_map(
                    self.grid.domain_dim
                ).size_local
    ...
    def print_constitutive_model(self) -> None:
        ...
        for elem_e in self.mat.elems_e:
            elems_e_list += (
                elem_e.name if len(elems_e_list) == 0 else ", " + elem_e.name
            )
        elems_ne_list = ""
        for elem_ne in self.mat.elems_ne:
            elems_ne_list += (
                elem_ne.name if len(elems_ne_list) == 0 else ", " + elem_ne.name
            )
        elems_th_list = ""
        for elem_th in self.mat.elems_th:
            elems_th_list += (
                elem_th.name if len(elems_th_list) == 0 else ", " + elem_th.name
            )

        n_elems = (
            len(self.mat.elems_e) + len(self.mat.elems_ne) + len(self.mat.elems_th)
        )
        if n_elems > 0:
            size = max(
                [len(elems_e_list), len(elems_ne_list), len(elems_th_list)]
            ) - len("List of elements")
            self.print_comment(" Constitutive model:")
            self.set_header_columns(
                ["Element type ", "List of elements" + " " * size], "left"
            )
            self.print_header()
    ...
    def print_output_info(self) -> None:
        ...
        self.set_header_columns(
            ["Location" + 10 * " ", "Field name      ", "Label name             "],
            "center",
        )
        self.print_header()
    ...
    def close(self) -> None:
        ...
        if MPI.COMM_WORLD.rank == 0:
            ...
            message = self.format_cell(
                f"Total time: {formatted_time} ({cpu_time} seconds)",
                full_width,
                "right",
            )
    ...
    def print_header(self) -> None:
        ...
        header_line = "| " + " | ".join(
            ...
            for col, w, align in zip(
                self.header_columns, self.widths, self.header_align
            )
        )  # + " |"
    ...
    def print_row(self, values: list[str]) -> None:
        ...
        row_line = "| " + " | ".join(
            ...
            for val, w, align, text_format in zip(
                values, self.widths, self.row_align, self.row_formats
            )
        )  # + " |"
    ...
    def format_cell(
        self, text, width: int, alignment: str, text_format: str = None
    ) -> None:
        """
