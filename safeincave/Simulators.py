...
class Simulator_GUI(Simulator):
    ...
    def initialize_material(self):
        ...
        for elem_name in self.input_file["constitutive_model"]["elastic"].keys():
            E = self.grid.get_parameter(
                self.input_file["constitutive_model"]["elastic"][elem_name][
                    "parameters"
                ]["E"]
            )
            nu = self.grid.get_parameter(
                self.input_file["constitutive_model"]["elastic"][elem_name][
                    "parameters"
                ]["nu"]
            )
    ...
    def run_equilibrium(self):
        ...
        for elem_name in self.input_file["constitutive_model"]["nonelastic"].keys():
            if self.input_file["constitutive_model"]["nonelastic"][elem_name]["active"]:
                if self.input_file["constitutive_model"]["nonelastic"][elem_name][
                    "equilibrium"
                ]:
                    if (
                        self.input_file["constitutive_model"]["nonelastic"][elem_name][
                            "type"
                        ]
                        == "KelvinVoigt"
                    ):
                        E = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["E"]
                        )
                        nu = self.grid.get_parameter(
                        ...
                        A = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["A"]
                        )
                        n = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["n"]
                        )
                        Q = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["Q"]
                        )
                        ...
                        mu_1 = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["mu_1"]
                        )
                        N_1 = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["N_1"]
                        )
                        ...
                        beta_1 = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["beta_1"]
                        )
                        beta = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["beta"]
                        )
                        m = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["m"]
                        )
                        gamma = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["gamma"]
                        )
                        alpha_0 = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["alpha_0"]
                        )
                        sigma_t = self.grid.get_parameter(
                            self.input_file["constitutive_model"]["nonelastic"][
                                elem_name
                            ]["parameters"]["sigma_t"]
                        )
                        desai = ViscoplasticDesai(
                            mu_1,
                            N_1,
                            a_1,
                            eta,
                        ...
                        elem_type = self.input_file["constitutive_model"]["nonelastic"][
                            elem_name
                        ]["type"]
        ...
        tc_equilibrium = TimeController(
            dt=dt, initial_time=0.0, final_time=tf, time_unit="second"
        )
        ...
        for b_name in self.input_file["boundary_conditions"].keys():
            ...
            if self.input_file["boundary_conditions"][b_name]["type"] == "neumann":
                bc = momBC.NeumannBC(
                    ...
                    direction=self.input_file["boundary_conditions"][b_name][
                        "direction"
                    ],
                    density=self.input_file["boundary_conditions"][b_name]["density"],
                    ref_pos=self.input_file["boundary_conditions"][b_name][
                        "reference_position"
                    ],
                    values=bc_values,
                ...
                bc = momBC.DirichletBC(
                    boundary_name=b_name,
                    component=self.input_file["boundary_conditions"][b_name][
                        "component"
                    ],
                    values=bc_values,
                    time_values=t_values,
        ...
        # Define simulator
        sim = Simulator_M(
            self.mom_eq, tc_equilibrium, outputs, compute_elastic_response=True
        )
        sim.run()
            ...
            if self.input_file["constitutive_model"]["nonelastic"][elem_name]["active"]:
                if (
                    ...
                    if not self.element_exist(elem_name):
                        ...
                        if self.input_file["simulation_settings"]["operation"][
                            "hardening"
                        ]:
                            # Compute initial hardening parameter
                            stress_to = numpy2torch(
                                self.mom_eq.sig.x.array.reshape(
                                    (self.mom_eq.n_elems, 3, 3)
                                )
                            )
