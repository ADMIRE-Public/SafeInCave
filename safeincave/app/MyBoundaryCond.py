...
class JSONBoundaryApp:
    ...
    def __init__(self, parent):
        ...
        self.boundary_combobox = ttk.Combobox(
            input_frame,
            state="readonly",
            values=list(self.json_data["boundary_conditions"].keys()),
        )
        ...
        self.type_combobox = ttk.Combobox(
            input_frame, values=["neumann", "dirichlet"], state="readonly"
        )
        ...
        self.component_combobox = ttk.Combobox(
            input_frame, values=["x", "y", "z"], state="readonly"
        )
        ...
        self.choose_point_combobox = ttk.Combobox(
            input_frame, values=self.time_list, state="readonly"
        )
        self.choose_point_combobox.grid(row=5, column=1, sticky="ew")
        self.choose_point_combobox.bind(
            "<<ComboboxSelected>>", self.update_prescribed_value
        )

        self.pbrowse_button = ttk.Button(
            input_frame, text="Browse", command=self.browse_csv
        )
    ...
    def SetBoundaryList(self, grid_boundary_names):
        ...
        updated_boundaries = {
            key: value
            for key, value in current_boundaries.items()
            if key in grid_boundary_names
        }
                ...
                with open(file_path, newline="", encoding="utf-8") as csvfile:
                    ...
                    if len(numbers) != len(self.time_list):
                        messagebox.showerror(
                            "Error", f"Time count and Number count are not equal!!!"
                        )
                    else:
    ...
    def update_combobox(self):
        ...
        self.boundary_combobox["values"] = list(
            self.json_data["boundary_conditions"].keys()
        )
    ...
    def update_time_list(self, new_time_list):
        if len(new_time_list) >= len(self.time_list):
            # Add zeros
            for boundary_name, boundary_data in self.json_data[
                "boundary_conditions"
            ].items():
                boundary_data["values"].extend(
                    [0.0] * (len(new_time_list) - len(self.time_list))
                )
        else:
            for boundary_name, boundary_data in self.json_data[
                "boundary_conditions"
            ].items():
                boundary_data["values"] = boundary_data["values"][
                    : (len(new_time_list) - len(self.time_list))
                ]
    ...
    def update_prescribed_value(self, event=None):
        ...
        if selected_boundary and selected_index >= 0:
            values = (
                self.json_data["boundary_conditions"]
                .get(selected_boundary, {})
                .get("values", [])
            )
            if selected_index < len(self.time_list):
                if selected_index < len(values):
                    ...
                    self.json_data["boundary_conditions"][selected_boundary]["values"][
                        selected_index
                    ] = 0
    ...
    def load_boundary_data(self, event):
        ...
        if selected_boundary:
            boundary_data = self.json_data["boundary_conditions"].get(
                selected_boundary, {}
            )
            ...
            if boundary_type == "neumann":
                ...
                self.reference_entry.insert(
                    0, boundary_data.get("reference_position", "")
                )
            else:
                # Set direction/component value
        ...
        if selected_boundary and new_direction:
            try:
                if selected_type == "neumann":
                    self.json_data["boundary_conditions"][selected_boundary][
                        "direction"
                    ] = int(self.values_map.get(new_direction, 0))
                else:
                    self.json_data["boundary_conditions"][selected_boundary][
                        "component"
                    ] = int(self.values_map.get(new_direction, 0))
    ...
    def update_density(self, event=None):
        ...
        if selected_boundary and new_density:
            try:
                self.json_data["boundary_conditions"][selected_boundary]["density"] = (
                    float(new_density)
                )
            except:
                self.json_data["boundary_conditions"][selected_boundary]["density"] = (
    ...
    def update_plot(self):
        ...
        if selected_boundary:
            ...
            if hasattr(self, "selected_index") and 0 <= self.selected_index < len(
                self.time_list
            ):
    ...
    def toggle_fields(self, event=None):
        ...
        if selected_boundary:
            self.json_data["boundary_conditions"][selected_boundary]["type"] = (
                selected_type
            )
    ...
    def on_press(self, event):
        if event.xdata is not None and event.ydata is not None:
            ...
            for i, (x, y) in enumerate(zip(self.time_list[: len(values)], values)):
                if (
                    abs(event.xdata - x) < threshold_x
                    and abs(event.ydata - y) < threshold_y
                ):
