...
class JSONConstitutiveApp:
    ...
    def _make_entry(self):
        ...
        var.trace_add(
            "write", lambda var_name, index, mode, e=entry: self.on_var_change(e)
        )
    ...
    def create_widgets(self):
        ...
        self.type_combo = ttk.Combobox(
            self.root,
            values=["Spring", "KelvinVoigt", "ViscoplasticDesai", "DislocationCreep"],
            state="readonly",
        )
        ...
        self.add_button = ttk.Button(
            self.root, text="Add Model", command=self.add_model
        )
        ...
        self.radio_true = tk.Radiobutton(
            self.root, text="True", variable=self.equi_var, value=True
        )
        self.radio_false = tk.Radiobutton(
            self.root, text="False", variable=self.equi_var, value=False
        )
        ...
        self.e_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.e_entry)
        )

        self.nu_label = ttk.Label(self.root, text="nu:")
        # self.nu_entry = ttk.Entry(self.root)
        self.nu_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.nu_entry)
        )

        self.eta_label = ttk.Label(self.root, text="eta:")
        # self.eta_entry = ttk.Entry(self.root)
        self.eta_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.eta_entry)
        )

        self.a_label = ttk.Label(self.root, text="A:")
        # self.a_entry = ttk.Entry(self.root)
        self.a_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.a_entry)
        )

        self.n_label = ttk.Label(self.root, text="n:")
        # self.n_entry = ttk.Entry(self.root)
        self.n_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.n_entry)
        )

        self.q_label = ttk.Label(self.root, text="Q:")
        # self.q_entry = ttk.Entry(self.root)
        self.q_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.q_entry)
        )

        self.t_label = ttk.Label(self.root, text="T:")
        # self.t_entry = ttk.Entry(self.root)
        self.t_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.t_entry)
        )

        # Labels and Entry fields for variables
        self.mu_1_label = ttk.Label(self.root, text="μ₁:")
        # self.mu_1_entry = ttk.Entry(self.root)
        self.mu_1_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.mu_1_entry)
        )

        self.N_1_label = ttk.Label(self.root, text="N₁:")
        # self.N_1_entry = ttk.Entry(self.root)
        self.N_1_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.N_1_entry)
        )

        self.n_desai_label = ttk.Label(self.root, text="n:")
        # self.n_desai_entry = ttk.Entry(self.root)
        self.n_desai_browse_button = tk.Button(
            self.root,
            text="Browse",
            command=lambda: self.browse_csv(self.n_desai_entry),
        )

        self.a_1_label = ttk.Label(self.root, text="a₁:")
        # self.a_1_entry = ttk.Entry(self.root)
        self.a_1_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.a_1_entry)
        )

        self.eta_desai_label = ttk.Label(self.root, text="η:")
        # self.eta_desai_entry = ttk.Entry(self.root)
        self.eta_desai_browse_button = tk.Button(
            self.root,
            text="Browse",
            command=lambda: self.browse_csv(self.eta_desai_entry),
        )

        self.beta_1_label = ttk.Label(self.root, text="β₁:")
        # self.beta_1_entry = ttk.Entry(self.root)
        self.beta_1_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.beta_1_entry)
        )

        self.beta_label = ttk.Label(self.root, text="β:")
        # self.beta_entry = ttk.Entry(self.root)
        self.beta_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.beta_entry)
        )

        self.m_label = ttk.Label(self.root, text="m:")
        # self.m_entry = ttk.Entry(self.root)
        self.m_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.m_entry)
        )

        self.gamma_label = ttk.Label(self.root, text="γ:")
        # self.gamma_entry = ttk.Entry(self.root)
        self.gamma_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.gamma_entry)
        )

        self.alpha_0_label = ttk.Label(self.root, text="α₀:")
        # self.alpha_0_entry = ttk.Entry(self.root)
        self.alpha_0_browse_button = tk.Button(
            self.root,
            text="Browse",
            command=lambda: self.browse_csv(self.alpha_0_entry),
        )

        self.k_v_label = ttk.Label(self.root, text="k_v:")
        # self.k_v_entry = ttk.Entry(self.root)
        self.k_v_browse_button = tk.Button(
            self.root, text="Browse", command=lambda: self.browse_csv(self.k_v_entry)
        )

        self.sigma_t_label = ttk.Label(self.root, text="σ_t:")
        # self.sigma_t_entry = ttk.Entry(self.root)
        self.sigma_t_browse_button = tk.Button(
            self.root,
            text="Browse",
            command=lambda: self.browse_csv(self.sigma_t_entry),
        )
        ...
        self.tree = ttk.Treeview(
            self.root,
            columns=("Category", "Name", "Type", "Edit", "Remove"),
            show="headings",
        )
        ...
        self.tree.grid(
            row=2, column=0, rowspan=6, columnspan=6, sticky="nsew", padx=5, pady=5
        )
        ...
        self.add_button = ttk.Button(
            self.root, text="Apply Changes", command=self.keep_changes
        )
    ...
    def add_entry_with_button(
        self, label, entry, button, row, column_label, column_entry, column_button
    ):
    ...
    def type_select_change(self):
        ...
        if selected_type == "Spring":
            self.add_entry_with_button(
                self.e_label, self.e_entry, self.e_browse_button, 1, 7, 8, 9
            )
            self.add_entry_with_button(
                self.nu_label, self.nu_entry, self.nu_browse_button, 2, 7, 8, 9
            )
            ...
            self.add_entry_with_button(
                self.eta_label, self.eta_entry, self.eta_browse_button, 3, 7, 8, 9
            )
        elif selected_type == "DislocationCreep":
            self.add_entry_with_button(
                self.a_label, self.a_entry, self.a_browse_button, 1, 7, 8, 9
            )
            self.add_entry_with_button(
                self.n_label, self.n_entry, self.n_browse_button, 2, 7, 8, 9
            )
            self.add_entry_with_button(
                self.q_label, self.q_entry, self.q_browse_button, 3, 7, 8, 9
            )
            self.add_entry_with_button(
                self.t_label, self.t_entry, self.t_browse_button, 4, 7, 8, 9
            )
        elif selected_type == "ViscoplasticDesai":
            self.add_entry_with_button(
                self.mu_1_label, self.mu_1_entry, self.mu_1_browse_button, 1, 7, 8, 9
            )
            self.add_entry_with_button(
                self.N_1_label, self.N_1_entry, self.N_1_browse_button, 2, 7, 8, 9
            )
            self.add_entry_with_button(
                self.n_desai_label,
                self.n_desai_entry,
                self.n_desai_browse_button,
                3,
                7,
                8,
                9,
            ...
            self.add_entry_with_button(
                self.a_1_label, self.a_1_entry, self.a_1_browse_button, 4, 7, 8, 9
            )
            self.add_entry_with_button(
                self.eta_desai_label,
                self.eta_desai_entry,
                self.eta_desai_browse_button,
                5,
                7,
                8,
                9,
            ...
            self.add_entry_with_button(
                self.beta_label, self.beta_entry, self.beta_browse_button, 7, 7, 8, 9
            )
            self.add_entry_with_button(
                self.m_label, self.m_entry, self.m_browse_button, 8, 7, 8, 9
            )
            self.add_entry_with_button(
                self.gamma_label, self.gamma_entry, self.gamma_browse_button, 9, 7, 8, 9
            )
            self.add_entry_with_button(
                self.alpha_0_label,
                self.alpha_0_entry,
                self.alpha_0_browse_button,
                10,
                7,
                8,
                9,
            ...
            self.add_entry_with_button(
                self.k_v_label, self.k_v_entry, self.k_v_browse_button, 11, 7, 8, 9
            )
            self.add_entry_with_button(
                self.sigma_t_label,
                self.sigma_t_entry,
                self.sigma_t_browse_button,
                12,
                7,
                8,
                9,
        ...
        if model_name and selected_type:
            ...
            def get_values(entry, param_name):
                try:
                    ...
                    if (
                        len_ != 1
                        and len_ != self.number_of_region
                        and len_ != self.number_of_elements
                    ):
                        messagebox.showerror(
                            "Warning", f"{param_name} values length is incorrect!"
                        )
                    if len(numbers) == 1:
                        return numbers[0]
                    return numbers
            ...
            if model_name != new_model_name:
                self.data["constitutive_model"][category1][new_model_name] = self.data[
                    "constitutive_model"
                ][category1].pop(model_name)
            ...
            messagebox.showwarning(
                "Input Error", "Please provide a name and select a type."
            )
        self.reset_colors()
    ...
    def keep_changes(self):
        if self.in_edit_mode:
            if self.confirm_keep_item_changes(self.item_keep_name, self.item_keep_type):
                ...
                self.Add_Keep_Changes(
                    self.item_keep_type, self.item_keep_name, new_model_name
                )
    ...
    def edit_item_popup(self, category, name):
        ...
        if model.get("type") == "Spring":
            ...
            self.e_entry.insert(
                0,
                model["parameters"].get("E", []) if "E" in model["parameters"] else "",
            )
            self.nu_entry.delete(0, tk.END)
            self.nu_entry.insert(
                0,
                model["parameters"].get("nu", [])
            ...
            self.a_entry.insert(
                0,
                model["parameters"].get("A", []) if "A" in model["parameters"] else "",
            )

            self.n_entry.delete(0, tk.END)
            self.n_entry.insert(
                0,
                model["parameters"].get("n", []) if "n" in model["parameters"] else "",
            )

            self.q_entry.delete(0, tk.END)
            self.q_entry.insert(
                0,
                model["parameters"].get("Q", []) if "Q" in model["parameters"] else "",
            )

            self.t_entry.delete(0, tk.END)
            self.t_entry.insert(
                0,
                model["parameters"].get("T", []) if "T" in model["parameters"] else "",
            )

        elif model.get("type") == "ViscoplasticDesai":
            # Grid setup and entry updates for each parameter
            self.mu_1_entry.delete(0, tk.END)
            self.mu_1_entry.insert(
                0,
                model["parameters"].get("mu_1", [])
                if "mu_1" in model["parameters"]
                else "",
            )

            self.N_1_entry.delete(0, tk.END)
            self.N_1_entry.insert(
                0,
                model["parameters"].get("N_1", [])
                if "N_1" in model["parameters"]
                else "",
            )

            self.n_desai_entry.delete(0, tk.END)
            self.n_desai_entry.insert(
                0,
                model["parameters"].get("n", []) if "n" in model["parameters"] else "",
            )

            self.a_1_entry.delete(0, tk.END)
            self.a_1_entry.insert(
                0,
                model["parameters"].get("a_1", [])
                if "a_1" in model["parameters"]
                else "",
            )

            self.eta_desai_entry.delete(0, tk.END)
            self.eta_desai_entry.insert(
                0,
                model["parameters"].get("eta", [])
                if "eta" in model["parameters"]
                else "",
            )

            self.beta_1_entry.delete(0, tk.END)
            self.beta_1_entry.insert(
                0,
                model["parameters"].get("beta_1", [])
                if "beta_1" in model["parameters"]
                else "",
            )

            self.beta_entry.delete(0, tk.END)
            self.beta_entry.insert(
                0,
                model["parameters"].get("beta", [])
                if "beta" in model["parameters"]
                else "",
            )
            ...
            self.m_entry.insert(
                0,
                model["parameters"].get("m", []) if "m" in model["parameters"] else "",
            )

            self.gamma_entry.delete(0, tk.END)
            self.gamma_entry.insert(
                0,
                model["parameters"].get("gamma", [])
                if "gamma" in model["parameters"]
                else "",
            )

            self.alpha_0_entry.delete(0, tk.END)
            self.alpha_0_entry.insert(
                0,
                model["parameters"].get("alpha_0", [])
                if "alpha_0" in model["parameters"]
                else "",
            )

            self.k_v_entry.delete(0, tk.END)
            self.k_v_entry.insert(
                0,
                model["parameters"].get("k_v", [])
                if "k_v" in model["parameters"]
                else "",
            )

            self.sigma_t_entry.delete(0, tk.END)
            self.sigma_t_entry.insert(
                0,
                model["parameters"].get("sigma_t", [])
                if "sigma_t" in model["parameters"]
                else "",
            )
    ...
    def confirm_and_remove(self, category, name):
        response = messagebox.askyesno(
            "Confirm Remove",
            f"Are you sure you want to remove '{name}' from {category}?",
        )
        if response:
    ...
    def confirm_keep_item_changes(self, category, name):
        response = messagebox.askyesno(
            "Confirm Edit",
            f"Are you sure you want to Keep Changes '{name}' from {category}?",
        )
        return response
