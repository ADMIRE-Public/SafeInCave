# Example 6: Hydrogen Cavern With Prescribed Pressure and Temperature

## Introduction

This example models hydrogen storage using prescribed pressure and temperature histories. The histories are read from `input_PT.json`, and the surrounding salt formation is solved with coupled heat diffusion and mechanics.

## Problem description

This section walks through `examples/thermomechanics/6_cavern_hydrogen/main.py` as an annotated, notebook-style listing. The code blocks follow the script in order and include all non-empty lines from `main.py`. Each block explains not only what the code does, but why the SafeInCave object is needed in the simulation workflow.
...
### Block 12: Create dislocation creep
...
### Block 25: Set boundary conditions
...
### Block 31: Calculate lithostatic pressure at the cavern's roof
...
```Python
    ...
    z_top = 660
    z_roof = 430
    p_roof = overburden + salt_density*abs(g)*(z_top - z_roof)
    print(0.5*p_roof)
```

### Block 32: Read flow rate values
...
```Python
    ...
    data_cavern = sf.Utils.read_json("input_PT.json")
```

### Block 33: Define cavern conditions
...
```Python
    ...
    cave_0 = caveBC.Cavern_PT(
                            grid = grid,
                            cavern_name = "Cavern",
                            fluid = "Hydrogen",
                            sym_scale = 4,
                            reference_point = [0.0, 0.0, z_roof],
                            P_values = data_cavern["pressure"],
                            T_values = data_cavern["temperature"],
...
### Block 34: Create output handlers
...
### Block 35: Continuation of the script
...
### Block 36: Continuation of the script
...
- `safeincave.Utils.read_json`: reads the pressure-temperature schedule.
- `safeincave.CavernBC.Cavern_PT`: applies prescribed hydrogen pressure and temperature histories at the cavern wall.
- `safeincave.CavernBC.CavernHandler`: passes cavern information to the simulator.
- `safeincave.LinearMomentumMixed`: solves the stabilized mixed mechanical formulation.
- `safeincave.HeatDiffusion`: solves the thermal field.
- `safeincave.Spring`, `safeincave.DislocationCreep`, `safeincave.PressureSolutionCreep`, and `safeincave.Thermoelastic`: define salt and overburden behavior.
- `safeincave.Simulator_Full`: runs the coupled simulation.
...
```bash
cd examples/thermomechanics/6_cavern_hydrogen
python main.py
```

The pressure-temperature input can be rebuilt from `PT_H2.csv` with:
...
This example shows how measured or externally generated hydrogen operation histories can drive a coupled SafeInCave model. It is useful when the fluid state is known and the main objective is the rock response.
