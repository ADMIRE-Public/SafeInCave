# Example 3: Cavern With Prescribed Pressure and Temperature

## Introduction

This example performs a fully coupled cavern simulation in which the cavern fluid state is prescribed through pressure and temperature histories. The rock mass responds mechanically and thermally to the cavern boundary conditions.

## Problem description

This section walks through `examples/thermomechanics/3_cavern_PT/main.py` as an annotated, notebook-style listing. The code blocks follow the script in order and include all non-empty lines from `main.py`. Each block explains not only what the code does, but why the SafeInCave object is needed in the simulation workflow.
...
### Block 21: Time settings for equilibrium stage
...
```Python
    ...
    time_ctrl = sf.TimeControllerParabolic(n_time_steps=10,
                                        initial_time=0.0,
                                        final_time=100,
                                        time_unit="day")
...
### Block 32: Define cavern conditions
...
```Python
    ...
    cave_1 = caveBC.Cavern_PT(
                            ...
                            P_values = [0.5*p_roof, 0.5*p_roof],
                            T_values = [280, 280],
                            h_conv = 5.0,
...
### Block 34: Continuation of the script
...
- `safeincave.HeatDiffusion`: solves heat diffusion in the surrounding rock.
- `safeincave.CavernBC.CavernHandler`: groups cavern boundary models.
- `safeincave.CavernBC.Cavern_PT`: prescribes cavern pressure and temperature histories.
- `safeincave.Spring`, `safeincave.DislocationCreep`, `safeincave.PressureSolutionCreep`, and `safeincave.Thermoelastic`: define the salt constitutive response.
- `safeincave.MomentumBC` and `safeincave.HeatBC`: define external mechanical and thermal boundary conditions.
- `safeincave.Simulator_Full`: couples mechanics, heat diffusion, and cavern-boundary evolution.
...
```bash
cd examples/thermomechanics/3_cavern_PT
python main.py
```

Results are written to `output/`. The helper script can be used to plot cavern-level quantities:
...
This example demonstrates the `Cavern_PT` workflow for cases where pressure and temperature histories are known in advance. It is useful for reproducing measured operation data or applying prescribed design scenarios.
