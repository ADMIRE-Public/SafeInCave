# Example 4: Water-Filled Cavern

## Introduction

This example runs a fully coupled thermomechanical simulation for a water-filled cavern. The cavern boundary is driven by a mass-flux model, while the surrounding rock follows thermoelastic, creep, and heat-diffusion behavior.

## Problem description

This section walks through `examples/thermomechanics/4_cavern_water/main.py` as an annotated, notebook-style listing. The code blocks follow the script in order and include all non-empty lines from `main.py`. Each block explains not only what the code does, but why the SafeInCave object is needed in the simulation workflow.
...
- `safeincave.LinearMomentumMixed`: defines the stabilized mixed mechanical problem.
- `safeincave.HeatDiffusion`: solves heat diffusion through the rock.
- `safeincave.CavernBC.Cavern_MassFlux`: computes cavern pressure and temperature from water mass exchange.
- `safeincave.CavernBC.CavernHandler`: passes the cavern model to the coupled simulator.
- `safeincave.Material`, `Spring`, `DislocationCreep`, `PressureSolutionCreep`, and `Thermoelastic`: define rock properties and constitutive behavior.
- `safeincave.Simulator_Full`: advances the fully coupled rock-fluid simulation.
...
```bash
cd examples/thermomechanics/4_cavern_water
python main.py
```

Outputs are stored in `output/`. To visualize the cavern time series, run:
...
This example shows how to couple a water cavern mass-balance model with heat diffusion and mechanical deformation. It can be adapted to investigate brine-filled cavern operations or thermal effects during liquid storage.
