...
The mechanical simulator only solves the momentum balance equation, so it either receives an object of class *LinearMomentum* or *LinearMomentumMixed*, along with the usual *TimeController*, *Outputs*, and *CavernHandler* objects. 
...
The *CavernHandler* object is optional. If a *None* object is provided, then either *Neumann* or *Dirichlet* must be prescribed for the cavern walls.

??? warning "Be mindful"

    Despite being allowed, prescribing displacements (*Dirichlet*) to the cavern wall doesn't make much sense, from a physical perspective. 
