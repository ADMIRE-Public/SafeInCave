...
The strain rate $\dot{\pmb{\varepsilon}}_i$, evaluated at the current time level $t+\Delta t$, depends on the stress tensor $\pmb{\sigma}$, which is still an unknown. Therefore, the problem must be linearized and solved iteratively in each time step. For this reason, for every variable at $t+\Delta t$ we indicate the **previous** iteration level with the superscript $k$ (previous iteration), and no superscript is used for the **current** iteration (superscript $k+1$ is supressed to simplify notation). Therefore, Eq. $\eqref{eq:eps_i_0}$ is rewritten as

$$
\pmb{\varepsilon}_i = \pmb{\varepsilon}_i^t + \phi_1 \dot{\pmb{\varepsilon}}_i^t + \phi_2 \dot{\pmb{\varepsilon}}_i^{k+1}
\label{eq:eps_i_1}
$$
...
$$
\pmb{\sigma} = \pmb{\sigma}_0 + \mathbb{C}_e : \left( \pmb{\varepsilon}^{k+1} - \pmb{\varepsilon}_{th} - \pmb{\varepsilon}_{ne}^t - \phi_1 \dot{\pmb{\varepsilon}}_{ne}^t - \phi_2 \dot{\pmb{\varepsilon}}_{ne}^{k+1}\right).
\label{eq:stress_2}
$$
...
\dot{\pmb{\varepsilon}}_i = \dot{\pmb{\varepsilon}}_i^{k} 
	+ \frac{\partial \dot{\pmb{\varepsilon}}_i^k}{\partial \pmb{\sigma}} : \delta \pmb{\sigma}
	+ \frac{\partial \dot{\pmb{\varepsilon}}_i^k}{\partial \omega_i} \delta \omega_i
...
$$
r_i = r_i^k + \frac{\partial r_i^k}{\partial \pmb{\sigma}} : \delta \pmb{\sigma} + \underbrace{\frac{\partial r_i^k}{\partial \omega_i}}_{h_i^k} \delta \omega_i = 0
	\quad \rightarrow \quad
	\delta \omega_i = - \frac{1}{h_i^k} \left( r_i^k + \frac{\partial r_i^k}{\partial \pmb{\sigma}} : \delta \pmb{\sigma} \right).
\label{eq:res_0}
$$
...
$$
\dot{\pmb{\varepsilon}}_i = \dot{\pmb{\varepsilon}}_i^{k} 
	+ \underbrace{\left( \frac{\partial \dot{\pmb{\varepsilon}}_i^k}{\partial \pmb{\sigma}} - \frac{1}{h_i^k} \frac{\partial \dot{\pmb{\varepsilon}}_i^k}{\partial \omega_i} \frac{\partial r_i^k}{\partial \pmb{\sigma}} \right)}_{\mathbb{G}_i^k} : \delta \pmb{\sigma}
	- \underbrace{\frac{r_i^k}{h_i^k} \frac{\partial \dot{\pmb{\varepsilon}}_i^k}{\partial \omega_i}}_{\mathbf{B}_i^k}
	\quad \rightarrow \quad
	\dot{\pmb{\varepsilon}}_i = \dot{\pmb{\varepsilon}}_i^{k} + \mathbb{G}_i^k : \delta \pmb{\sigma} - \mathbf{B}_i^k.
...
$$
\dot{\pmb{\varepsilon}}_{ne}
	=
	\dot{\pmb{\varepsilon}}_{ne}^{k} + \mathbb{G}_{ne}^k : \delta \pmb{\sigma} - \mathbf{B}_{ne}^k,
\label{eq:eps_rate_ne_0}
$$

where $\mathbb{G}_{ne}^k = \sum_{i=1}^{N_{ne}} \mathbb{G}_i^k$ and $\mathbf{B}_{ne}^k = \sum_{i=1}^{N_{ne}} \mathbf{B}_i^k$.

Finally, substituting Eq. $\eqref{eq:eps_rate_ne_0}$ into Eq. $\eqref{eq:stress_2}$ leads to

$$
\pmb{\sigma} = \mathbb{C}_T : \left[ 
		\pmb{\varepsilon}_0 
		+ \pmb{\varepsilon}^{k+1} 
		- \pmb{\varepsilon}_{th}
		- \pmb{\varepsilon}_{ne}^k
		+ \phi_2 \left( \mathbb{G}_{ne}^k : \pmb{\sigma}^k + \mathbf{B}_{ne}^k \right)
	\right]
\label{eq:stress_3}
$$
...
\begin{align}
    ...
    \\
    &\mathbb{C}_T^k = \left( \mathbb{C}_0^{-1} + \phi_2 \mathbb{G}_{ne}^k \right)^{-1} \label{eq:CT}
\end{align}
$$
...
$$
\pmb{\varepsilon}^k_\text{rhs} = \pmb{\varepsilon}_{ne}^k - \phi_2 \left( \mathbb{G}_{ne}^k : \pmb{\sigma}^k + \mathbf{B}_{ne}^k \right).
\label{eq:eps_rhs}
$$
...
$$
\nabla \cdot \mathbb{C}_T^k : \pmb{\varepsilon} = \rho\mathbf{g} + \nabla \cdot \mathbb{C}_T^k : \left( \pmb{\varepsilon}_\text{rhs}^k + \pmb{\varepsilon}_{th} - \pmb{\varepsilon}_0 \right).
\label{eq:mom_1}
$$
