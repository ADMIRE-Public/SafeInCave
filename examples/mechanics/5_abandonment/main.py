...
def main():
	...
	bc_cavern = momBC.SpecialNeumannBC(
		boundary_name="Cavern",
		direction=2,          # hydrostatic axis
		density=gas_density,  # for head term ρ g (H - z)
		ref_pos=430.0,        # H
		p0=p_gas,             # initial cavern pressure
		g=g_vec[2],
	)
	...
	# Define simulator
	sim = sf.Simulator_M(mom_eq, tc_equilibrium, outputs, True, abandonment_mode="equilibrium")
	sim.run()


	p_last, V_last = mom_eq.bc.get_abandonment_state()  # (p_curr, V_curr) from SpecialNeumannBC
	...
	bc_cavern = momBC.SpecialNeumannBC(boundary_name="Cavern",
		direction=2,          # hydrostatic axis
		density=gas_density,  # for head term ρ g (H - z)
		ref_pos=430.0,        # H
		p0=p_gas,             # initial cavern pressure
		g=g_vec[2],
		p_threshold=10.1*ut.MPa)  # pressure threshold for abandonment soft shut-in)
	...
	# push the seed into the freshly-set handler
	mom_eq.bc.last_logged_pressure = float(p_last)
	mom_eq.bc.last_logged_volume   = float(V_last)

	# keep the internal special-BC consistent
	if mom_eq.bc.special_neumann_boundaries:
		bc0 = mom_eq.bc.special_neumann_boundaries[0]
		bc0.p_curr  = bc0._last_P = float(p_last)
		bc0.V_curr  = bc0._last_V = float(V_last)

	mom_eq.bc.set_abandonment_mode("coupled")
	...
	# Define simulator
	sim = sf.Simulator_M(mom_eq, tc_operation, outputs, False, abandonment_mode="coupled")
	sim.run()
