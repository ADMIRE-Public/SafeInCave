...
def plot_stress_path(ax, output_folder):
    ...
    ax.scatter(
        p_path[0], q_path[0], c="white", edgecolors="black", zorder=5, label="t=0"
    )
    ...
    ax.plot(p_csl, M_CSL * p_csl, "--", color="dimgray", linewidth=0.8, label="CSL")
...
def main():
    ...
    fig.subplots_adjust(
        top=0.96, bottom=0.09, left=0.075, right=0.985, hspace=0.30, wspace=0.28
    )
