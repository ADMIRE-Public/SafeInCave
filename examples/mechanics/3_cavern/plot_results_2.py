...
from PostProcessingTools_2 import find_closest_point, build_mapping, read_node_vector, read_cell_scalar
import numpy as np
...
class WallProfileData():
	def __init__(self, output_folder, scale=1):
		...
		points, self.time_list, u_field = read_node_vector(os.path.join(output_folder, "u", "u.xdmf"))
		...
		mapping = build_mapping(points_msh, points)
...
class SubsidenceData():
	def __init__(self, output_folder):
		points, self.time_list, u_field = read_node_vector(os.path.join(output_folder, "u", "u.xdmf"))
		top_idx = find_closest_point([0,0,660], points)
...
class StressPath():
	def __init__(self, output_folder, probes):
		self.probes = probes
		self.points, self.time_list, self.p_elems = read_cell_scalar(os.path.join(output_folder, "p_elems", "p_elems.xdmf"))
		self.points, self.time_list, self.q_elems = read_cell_scalar(os.path.join(output_folder, "q_elems", "q_elems.xdmf"))

		self.p_probes = np.zeros((probes.shape[0], self.time_list.size))
		self.q_probes = np.zeros((probes.shape[0], self.time_list.size))
		for i, probe in enumerate(probes):
			idx = find_closest_point(probe, self.points)
