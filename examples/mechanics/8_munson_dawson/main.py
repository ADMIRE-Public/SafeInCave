...
def build_munson_dawson(mom_eq, scenario, E0, nu0):
    ...
    if scenario == "A":
        ...
        nmd = 4.99
        A_md = (18.31 * (1e-6) ** nmd / sec_per_year) * to.ones(
            mom_eq.n_elems, dtype=to.float64
        )
        ...
        nmd = 5.6897
        A_md = (17.28 * (1e-6) ** nmd / sec_per_year) * to.ones(
            mom_eq.n_elems, dtype=to.float64
        )
...
def initialize_md_steady_state(md, mom_eq, T_kelvin):
    ...
    stress = ut.numpy2torch(mom_eq.sig.x.array.reshape((mom_eq.n_elems, 3, 3)))
...
def build_bcs(
    ...
    bc_west = momBC.DirichletBC(
        boundary_name="West", component=0, values=[0.0, 0.0], time_values=[0.0, t_final]
    )
...
def main():
    ...
    cavern_values = [p_max, p_min, p_min, p_max, p_min, p_min, p_max]
    cavern_times = [
        0.0,
        2.0 * ut.day,
        13.0 * ut.day,
