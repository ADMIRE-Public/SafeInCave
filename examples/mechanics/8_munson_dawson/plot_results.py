"""
Plot results from the Munson-Dawson example simulation.

Mirrors examples/mechanics/nobian/Old/plot_results.py:
    - cavern wall-profile (initial vs deformed)
    - convergence (% volume loss) vs time
    - subsidence at the top boundary
    - p-q stress paths at probe points around the cavern wall
...
def create_layout():
    ...
    fig.subplots_adjust(
        top=0.975, bottom=0.120, left=0.060, right=0.986, hspace=0.44, wspace=0.64
    )
...
class WallProfileData:
    ...
    def __trapezoidal_volume(self, x, y):
        """Volume of a solid of revolution (around y=0 axis) by trapezoidal rule."""
        volume = 0.0
        n = len(x)
        for i in range(1, n):
            R = 0.5 * (y[i] + y[i - 1])
...
def show_metadata(ax_mesh, ax_model, ax_solver, output_folder):
    ...
    for i, line in enumerate(log):
        if "| Mesh info:" in line:
            ...
            ax_mesh.text(
                0, h - 1 * dh, f"- Location: {location}", size=10, fontname="serif"
            )
            ax_mesh.text(
                0,
                h - 2 * dh,
                f"- Number of elems: {n_elems}",
                size=10,
                fontname="serif",
            )
            ax_mesh.text(
            ...
            ax_solver.text(
                0, h - 0 * dh, "Simulation Info: ", size=12, fontname="serif"
            )
            ax_solver.text(
                0,
                h - 1 * dh,
                f"- Solver: {ksp_solver}, PC: {ksp_pc}",
                size=10,
                fontname="serif",
            )
            ax_solver.text(
                0,
                h - 2 * dh,
                f"- tol: {tol}, max_ite: {max_ite}",
                size=10,
                fontname="serif",
            )
            ax_solver.text(
                0, h - 3 * dh, f"- CPU Time: {cpu_time}", size=10, fontname="serif"
            )
            ax_solver.axis("off")
            ...
            ax_model.text(
                0, h - 0 * dh, "Constitutive model: ", size=12, fontname="serif"
            )
            ax_model.text(
                0, h - 1 * dh, f"- Elastic: {elastic_elems}", size=10, fontname="serif"
            )
            ax_model.text(
                0,
                h - 2 * dh,
                f"- Non-elastic: {nonelastic_elems}",
                size=10,
                fontname="serif",
            )
            ax_model.text(
                0,
                h - 3 * dh,
                f"- Thermoelastic: {thermoelastic_elems}",
                size=10,
                fontname="serif",
            )
            ax_model.axis("off")
...
def plot_subsidence(ax, sub: SubsidenceData, t_step: int):
    ...
    ax.scatter(
        sub.time_list[t_step] / day,
        100 * sub.subsidence[t_step],
        c="white",
        edgecolors="black",
        zorder=10000,
    )
...
class StressPath:
    def __init__(self, output_folder, probes):
        ...
        self.points, self.time_list, self.q_elems = post.read_cell_scalar(
...
def cavern_pressure_schedule():
    """Cavern-pressure schedule used in the operation phase of main.py.

    Two 15-day cycles between p_max=10 MPa and p_min=7 MPa:
    ramp down (2 d) -> hold low (11 d) -> ramp up (2 d), repeat."""
    times_days = np.array([0.0, 2.0, 13.0, 15.0, 17.0, 28.0, 30.0])
    p_mpa = np.array([10.0, 7.0, 7.0, 10.0, 7.0, 7.0, 10.0])
    return times_days, p_mpa


def plot_pressure_schedule(ax, time_list, t_step: int):
    times_days, p_mpa = cavern_pressure_schedule()
    ax.plot(times_days, p_mpa, "-", color="seagreen", linewidth=2.0)
    t_now = time_list[t_step] / day
    p_now = np.interp(t_now, times_days, p_mpa)
    ax.scatter(t_now, p_now, c="white", edgecolors="black", zorder=10000)
    ax.set_xlabel("Time (days)", size=12, fontname="serif")
...
def main():
    ...
    plot_pressure_schedule(ax30, stress_path.time_list, t_step=0)
    ...
    def update_plot(val):
        ...
        ax30.cla()
        plot_pressure_schedule(ax30, stress_path.time_list, t_step=t_step)
