...
def main():
    ...
    tc_eq = sf.TimeControllerParabolic(
        n_time_steps=20, initial_time=0.0, final_time=10, time_unit="day"
    )
    ...
    bc_west_salt = momBC.DirichletBC(
        boundary_name="West_salt",
        component=0,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )
    bc_west_ovb = momBC.DirichletBC(
        boundary_name="West_ovb",
        component=0,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )

    bc_east_salt = momBC.DirichletBC(
        boundary_name="East_salt",
        component=0,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )
    bc_east_ovb = momBC.DirichletBC(
        boundary_name="East_ovb",
        component=0,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )
    ...
    bc_south_salt = momBC.DirichletBC(
        boundary_name="South_salt",
        component=1,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )
    bc_south_ovb = momBC.DirichletBC(
        boundary_name="South_ovb",
        component=1,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )

    bc_north_salt = momBC.DirichletBC(
        boundary_name="North_salt",
        component=1,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )
    bc_north_ovb = momBC.DirichletBC(
        boundary_name="North_ovb",
        component=1,
        values=[0.0, 0.0],
        time_values=[0.0, tc_eq.t_final],
    )
    ...
    # Define simulator
    sim = sf.Simulator_M(
        eq_mom=mom_eq, t_control=tc_eq, outputs=outputs, compute_elastic_response=True
    )
    sim.run()
    ...
    bc_top = momBC.NeumannBC(
        ...
        ref_pos=z_surface,
        values=[0, 0],
        time_values=[0, tc_op.t_final],
        g=g_vec[2],
    )

    p_values = 3 * [0.8 * p_roof, 0.8 * p_roof, 0.2 * p_roof, 0.2 * p_roof] + [
        0.8 * p_roof
    ]
