...
# from safeincave.Utils import project
import os
import sys
import numpy as np
...
class Permeability(sf.PermeabilityBase):
	def __init__(self, n_elems: int, p: do.fem.function.Function):
...
def main():
	...
	output_folder = os.path.join("output", "case_0", "nonlinear")

	# Define equation
	mass_eq = sf.MassPorousMedia(grid, is_linear=False)

	# Define solver
	solver_heat = PETSc.KSP().create(grid.mesh.comm)
	solver_heat.setType("gmres")
